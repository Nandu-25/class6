...
class ChatRoomConsumer(AsyncWebsocketConsumer):
    ...
    def get_seat_number(self, user):
        ...
        if user.role.name == "student":
            ...
            return None
    @database_sync_to_async
    def get_enro_number(self, user):
        
        if user.role.name == "student":
            c1 = Q(student = user)
            room = Room.objects.get(id = self.room_name)
            c2 = Q(room = room)
            seat = Seat.objects.get(c1 & c2)
            enro = seat.enroll
            enro = str(enro)
            
            return enro # Assuming seat number is stored in the 'selected' field
        else:
            return None
    ...
    async def connect(self):
        ...
        print(seat_number)
        enro = await self.get_enro_number(user)
        print(enro)
        await self.update_user(user,rn)
        await self.update_teacher(user,rn)
        
        await self.channel_layer.group_send(
                self.room_group_name,
                {
                    'type':'cameBack_message',
                    'he_is_back':"back",
    ...
    async def disconnect(self, close_code):
        ...
        await self.delete_teacher(user,rn)
        await self.channel_layer.group_send(
                self.room_group_name,
                {
                    'type':'left_message',
    ...
    async def cameBack_message(self, event):
        print("came back message sent")
        he_left= event['he_is_back']
        seat_id = event['seat_id_name']
        enrof = event['enrof']
        role = event['role']
        await self.send(text_data=json.dumps({
    ...
    @sync_to_async
    def delete_teacher(self,user,room):
        
        room = Room.objects.get(id=room) 
        teacher = room.teacher_online
        if user.role.name == "teacher":
            if teacher:
                room.teacher_online=None
                room.save()
    ...
    @sync_to_async 
    def update_user(self,user,room_id):
        ...
        if user.role.name == "student":
            ...
            seat1 = Seat.objects.get(c1 & c2)
        # room = Room.objects.get(id=room) 
            seat1.is_online = True
        
            seat1.save()
    @sync_to_async 
    def update_teacher(self,user,room_id):
        
        if user.role.name == "teacher":
            room = Room.objects.get(id = room_id)
            teacher = room.teacher_online
            if not teacher:
                room.teacher_online = user
                room.save()
